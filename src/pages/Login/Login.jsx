...
const Login = () => {
  const {signIn, setLoading, resetPassword, signInWithGoogle, loading} = useAuth();
  ...
  const handleSignInUser = async (e) => {
      ...
      try{
        setLoading(true) // for spanner
        // for  signin user  -
        const result = await signIn(email, password)
        console.log(result);
        navigate(from)
        toast.success('SignIp Success')
    ...
    try{
      if(!email) return toast.error('Write your email first')
      resetPassword(email)
